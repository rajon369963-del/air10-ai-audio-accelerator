// Background Service Worker for CIVEX WhisperDub Local
...
const client = new LimbClient({ toolId: "whisperdub-local" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX WhisperDub Local] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("whisperdub-local", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("whisperdub-local", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "whisperdub-local", result: res });
    });
    return true;
  }
