// Background Service Worker for CIVEX SnippetVault
...
const client = new LimbClient({ toolId: "snippetvault-dense" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX SnippetVault] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("snippetvault-dense", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("snippetvault-dense", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "snippetvault-dense", result: res });
    });
    return true;
  }
