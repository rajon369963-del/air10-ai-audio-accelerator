// Background Service Worker for CIVEX GainMaximizer 600%
...
const client = new LimbClient({ toolId: "gain-maximizer" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX GainMaximizer 600%] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("gain-maximizer", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("gain-maximizer", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "gain-maximizer", result: res });
    });
    return true;
  }
