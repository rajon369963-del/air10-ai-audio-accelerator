// Background Service Worker for CIVEX FSRS QuickRecall
...
const client = new LimbClient({ toolId: "fsrs-quickrecall" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX FSRS QuickRecall] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("fsrs-quickrecall", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("fsrs-quickrecall", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "fsrs-quickrecall", result: res });
    });
    return true;
  }
