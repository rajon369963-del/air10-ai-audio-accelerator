// Background Service Worker for CIVEX One-Sentence Summarizer
...
const client = new LimbClient({ toolId: "one-sentence-summary" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX One-Sentence Summarizer] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("one-sentence-summary", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("one-sentence-summary", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "one-sentence-summary", result: res });
    });
    return true;
  }
