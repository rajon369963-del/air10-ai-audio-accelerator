// Background Service Worker for CIVEX TabSync Vector
...
const client = new LimbClient({ toolId: "tabsync-vector" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX TabSync Vector] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("tabsync-vector", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("tabsync-vector", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "tabsync-vector", result: res });
    });
    return true;
  }
