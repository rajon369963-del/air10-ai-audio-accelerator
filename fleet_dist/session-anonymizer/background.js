// Background Service Worker for CIVEX SessionAnonymizer
...
const client = new LimbClient({ toolId: "session-anonymizer" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX SessionAnonymizer] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("session-anonymizer", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("session-anonymizer", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "session-anonymizer", result: res });
    });
    return true;
  }
