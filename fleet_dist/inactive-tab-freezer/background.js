// Background Service Worker for CIVEX Inactive Tab Freezer
...
const client = new LimbClient({ toolId: "inactive-tab-freezer" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX Inactive Tab Freezer] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("inactive-tab-freezer", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("inactive-tab-freezer", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "inactive-tab-freezer", result: res });
    });
    return true;
  }
