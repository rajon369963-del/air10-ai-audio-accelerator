// Background Service Worker for CIVEX ContextSnip 250B
...
const client = new LimbClient({ toolId: "contextsnip-250" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX ContextSnip 250B] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("contextsnip-250", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("contextsnip-250", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "contextsnip-250", result: res });
    });
    return true;
  }
