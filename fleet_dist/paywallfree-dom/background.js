// Background Service Worker for CIVEX PaywallFree DOM Extractor
...
const client = new LimbClient({ toolId: "paywallfree-dom" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX PaywallFree DOM Extractor] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("paywallfree-dom", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("paywallfree-dom", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "paywallfree-dom", result: res });
    });
    return true;
  }
