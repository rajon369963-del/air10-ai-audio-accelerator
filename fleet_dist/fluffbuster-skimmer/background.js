// Background Service Worker for CIVEX FluffBuster Skimmer
...
const client = new LimbClient({ toolId: "fluffbuster-skimmer" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX FluffBuster Skimmer] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("fluffbuster-skimmer", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("fluffbuster-skimmer", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "fluffbuster-skimmer", result: res });
    });
    return true;
  }
