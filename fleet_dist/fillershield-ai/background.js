// Background Service Worker for CIVEX FillerShield AI
...
const client = new LimbClient({ toolId: "fillershield-ai" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX FillerShield AI] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("fillershield-ai", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("fillershield-ai", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "fillershield-ai", result: res });
    });
    return true;
  }
