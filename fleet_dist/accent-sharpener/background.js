// Background Service Worker for CIVEX AccentSharpener EQ
...
const client = new LimbClient({ toolId: "accent-sharpener" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX AccentSharpener EQ] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("accent-sharpener", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("accent-sharpener", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "accent-sharpener", result: res });
    });
    return true;
  }
