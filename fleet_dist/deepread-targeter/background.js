// Background Service Worker for CIVEX DeepRead Targeter
...
const client = new LimbClient({ toolId: "deepread-targeter" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX DeepRead Targeter] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("deepread-targeter", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("deepread-targeter", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "deepread-targeter", result: res });
    });
    return true;
  }
