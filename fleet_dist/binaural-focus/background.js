// Background Service Worker for CIVEX Binaural FocusEngine
...
const client = new LimbClient({ toolId: "binaural-focus" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX Binaural FocusEngine] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("binaural-focus", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("binaural-focus", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "binaural-focus", result: res });
    });
    return true;
  }
