// Background Service Worker for CIVEX DataLeak Sentinel
...
const client = new LimbClient({ toolId: "dataleak-sentinel" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX DataLeak Sentinel] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("dataleak-sentinel", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("dataleak-sentinel", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "dataleak-sentinel", result: res });
    });
    return true;
  }
