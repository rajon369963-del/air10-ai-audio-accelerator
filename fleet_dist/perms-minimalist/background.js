// Background Service Worker for CIVEX PermsMinimalist
...
const client = new LimbClient({ toolId: "perms-minimalist" });
const telemetry = new ZeroTrackerTelemetry();

chrome.runtime.onInstalled.addListener(() => {
  client.connect().then(info => {
    console.log("[CIVEX PermsMinimalist] Initialized in " + info.mode + " mode.");
    telemetry.trackMetric("perms-minimalist", "INSTALL_SUCCESS", 1.0);
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "EXECUTE_MICRO_TOOL") {
    client.executeTask(request.task || "DEFAULT", request.payload || {}).then(res => {
      telemetry.trackMetric("perms-minimalist", "TASK_EXECUTE", 1.0);
      sendResponse({ success: true, toolId: "perms-minimalist", result: res });
    });
    return true;
  }
